...
import com.example.springb.service.UserConfigService;
...
public class FeishuController {
    ...
    public Result uploadWithUserToken(
            HttpServletRequest request,
            @RequestHeader(value = "X-User-Token", required = false) String userAccessToken) {
        ...
        try {
            return uploadFileToFeishuWithToken(file, fileName, resolveUserAccessToken(request, userAccessToken));
        } catch (Exception e) {
            log.error("上传异常", e);
            return Result.error("上传异常: " + e.getMessage());
        }
    ...
    public Result getCurrentUserFeishuToken(HttpServletRequest request) {
        Integer currentUserId = getCurrentUserId(request);
        JSONObject data = new JSONObject();
        data.set("token", userConfigService.getFeishuUserToken(currentUserId));
        return Result.success(data);
    }

    @PutMapping("/user-token")
    public Result saveCurrentUserFeishuToken(HttpServletRequest request, @RequestBody FeishuTokenRequest feishuTokenRequest) {
        Integer currentUserId = getCurrentUserId(request);
        userConfigService.saveFeishuUserToken(currentUserId, feishuTokenRequest == null ? "" : feishuTokenRequest.getToken());
    ...
    private String resolveUserAccessToken(HttpServletRequest request, String userAccessToken) {
        if (StrUtil.isNotBlank(userAccessToken)) {
            return userAccessToken.trim();
        }

        Integer currentUserId = getCurrentUserId(request);
        String savedToken = userConfigService.getFeishuUserToken(currentUserId);
        if (StrUtil.isBlank(savedToken)) {
            throw new CustomerException("400", "请先配置飞书 Token");
        }
        return savedToken;
    }
    ...
    public static class FeishuTokenRequest {
