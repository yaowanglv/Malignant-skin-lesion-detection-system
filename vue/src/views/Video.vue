<template>
  <div class="video-container">
    ...
    <div class="page-header">
      <h2>皮肤恶性病灶视频流辅助检测与分析</h2>
      <p class="subtitle">使用深度学习模型与AI大模型对皮肤镜视频进行逐帧病变检测与分析</p>
    </div>
    ...
    <div class="operation-area">
      ...
      <div class="operation-row">
        ...
        <el-upload
          ...
          :action="`${getBackendUrl()}/video/upload`"
          :data="uploadData"
...
import { Upload, VideoPlay, ChatLineRound, Loading, Download } from '@element-plus/icons-vue'
...
const originalVideoUrl = computed(() => {
  ...
  return `${getBackendUrl()}/files/${currentRecord.value.originalVideoUrl}`
})
...
const resultVideoUrl = computed(() => {
  ...
  return `${getBackendUrl()}/files/${currentRecord.value.resultVideoUrl}`
})
...
const defectDistColor = (type) => {
  ...
  if (lowerType.includes('benign')) return '#3b82f6'
  if (lowerType.includes('malignant')) return '#dc2626'
  if (lowerType.includes('normal')) return '#2f8f5b'
  return '#687782'
}
...
const createAiAnalysisCanvas = (record) => {
  ...
  ctx.strokeStyle = '#D9747E'; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(padding, 60); ctx.lineTo(width - padding, 60); ctx.stroke()
  ...
  lines.forEach((line) => {
    ...
    if (!isDisclaimer && text.includes('：')) {
      ...
      drawText(ctx, text, padding, y, { color: isDisclaimer ? '#c2413b' : '#1A2D3D', font: `${isDisclaimer || line.isSectionTitle ? '700 ' : ''}14px "Microsoft YaHei", "SimHei", Arial, sans-serif` })
    }
...
const createReportHeaderCanvas = (record, data) => {
  ...
  drawText(ctx, '皮肤癌视频检测分析报告', width / 2, 16, { align: 'center', color: '#1A2D3D', font: '700 24px "Microsoft YaHei", "SimHei", Arial, sans-serif' })
  drawText(ctx, '大模型驱动视频流皮肤恶性病灶实时辅助研判系统', width / 2, 52, { align: 'center', color: '#687782', font: '12px "Microsoft YaHei", "SimHei", Arial, sans-serif' })
  ctx.strokeStyle = '#2A7D8C'; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(30, 76); ctx.lineTo(width - 30, 76); ctx.stroke()
  ...
  if (data) {
    ctx.fillStyle = '#F7FAFC'; ctx.fillRect(30, 125, width - 60, 0)
    ctx.strokeStyle = '#E2EAF0'; ctx.lineWidth = 1
  }
  return canvas
}
...
const exportVideo = async () => {
  ...
  try {
    const response = await request.get(`/video/download/${currentRecord.value.id}`, {
      responseType: 'blob'
    })
    const fallbackName = currentRecord.value.originalVideoName || `result_video_${currentRecord.value.id}.mp4`
    ...
    console.error('下载检测视频失败', error)
    ElMessage.error('下载失败，请检查登录状态或后端服务')
  }
}
...
const loadRecordFromRoute = (recordId) => {
  ...
  } else if (shouldAutoRefreshPage('video')) {
...
const loadModels = async () => {
  ...
  try {
    const configRes = await fetchModels('video')
...
const loadDefaultThreshold = async () => {
  try {
    confThreshold.value = await getThreshold('video', 0.6)
  } catch (error) {
...
onActivated(() => {
  if (!hasMounted || !shouldAutoRefreshPage('video')) return
  resetPageState()
  loadRecordFromRoute(route.query.recordId)
  loadModels()
  loadDefaultThreshold()
