...
GENERATED_VIDEO_FOLDER = os.path.join(BASE_DIR, 'static', 'generated_videos')
MAX_BATCH_IMAGE_COUNT = 500
...
# 默认模型根目录（仓库内业务权重）
DEFAULT_MODEL_ROOT = os.path.join(BASE_DIR, 'models')

skin_class_mapping = {
    0: 'Benign-skin-cancer',
    1: 'Malignant-skin-cancer'
}


def resolve_skin_class(cls_id, raw_name):
    """Resolve skin numeric fallback classes when exported metadata is missing."""
    ...
    if numeric_text.isdigit() and int(numeric_text) in skin_class_mapping:
        return skin_class_mapping[int(numeric_text)]

    if cls_id in skin_class_mapping and raw_text in {'', str(cls_id), f'class{cls_id}'}:
        return skin_class_mapping[cls_id]
...
def normalize_video_dimensions(width, height):
    """H.264/yuv420p需要偶数宽高，统一修正为浏览器友好的尺寸"""
...
def find_ffmpeg_executable():
    """优先使用系统ffmpeg；没有时使用imageio-ffmpeg内置二进制"""
...
def transcode_mp4_for_browser(source_path, target_path):
    """将OpenCV生成的MP4转为浏览器稳定支持的H.264 + yuv420p + faststart"""
    ffmpeg_path = find_ffmpeg_executable()
    if not ffmpeg_path:
        print('[images_to_video] 未找到ffmpeg，MP4将使用OpenCV默认编码输出（浏览器可能无法播放）')
        return False

    print(f'[images_to_video] 使用ffmpeg转码H.264: {ffmpeg_path}')
    ...
    try:
        ...
        if result.returncode != 0:
            print(f'[images_to_video] ffmpeg转码失败 (returncode={result.returncode}): {result.stderr[:300]}')
            return False
        if os.path.exists(temp_target) and os.path.getsize(temp_target) > 0:
            os.replace(temp_target, target_path)
            print(f'[images_to_video] H.264转码成功: {os.path.getsize(target_path)} bytes')
            return True
        else:
            print(f'[images_to_video] 转码输出文件为空或不存在: {temp_target}')
            return False
    except Exception as exc:
        print(f'[images_to_video] ffmpeg转码异常，回退OpenCV默认MP4: {exc}')
    finally:
...
def normalize_resolution(resolution):
    """解析前端传入的分辨率参数"""
    resolution = str(resolution or 'original').lower()
...
def read_image_file(file_storage):
    ...
    if len(img_bytes) > MAX_BATCH_IMAGE_SIZE:
        raise ValueError(f'{file_storage.filename} 超过50MB限制')
    nparr = np.frombuffer(img_bytes, np.uint8)
    return cv2.imdecode(nparr, cv2.IMREAD_COLOR)


def build_public_url(path):
    """将本地相对路径转为服务可访问URL（使用请求来源地址，适配本地和Jetson部署）"""
    normalized = os.path.relpath(path, os.path.dirname(os.path.abspath(__file__))).replace('\\', '/')
    ...
    try:
        ...
        if result.boxes is not None:
            ...
            for i, box in enumerate(boxes):
                ...
                cls_name = resolve_skin_class(cls_id, result.names.get(cls_id, str(cls_id)))

                # 映射皮肤缺陷类型
                mapped_type = skin_defect_mapping.get(cls_name, cls_name)

                # 非映射类别过滤：不在皮肤癌映射中的类别视为正常
                if mapped_type not in ['Benign-skin-cancer', 'Malignant-skin-cancer']:
                    continue

                defect_type_cn = skin_defect_cn_mapping.get(mapped_type, mapped_type)
        ...
        if segment_mode:
            ...
            result.names = {
                k: skin_defect_mapping.get(resolve_skin_class(k, v), resolve_skin_class(k, v))
                for k, v in result.names.items()
            }

            # 使用ultralytics的绘图功能
            # result.plot() 返回 BGR numpy 数组 (OpenCV 格式)，需要转为 RGB 再创建 PIL Image
...
def list_models():
    """列出可用的模型文件（默认扫描 detect_service/models）"""
...
def images_to_video():
    ...
    请求参数:
    - images: 多个图片文件
    - duration_per_image: 每张图片显示时长（秒），范围0.1-5.0，默认0.3
    - output_format: mp4/avi/webm，默认mp4
    - resolution: original/1920x1080/1280x720/640x480，默认original
    """
    output_path = None
    try:
        ...
        if duration_per_image < 0.1 or duration_per_image > 5.0:
            return jsonify({'code': 400, 'msg': '每张图时长需在0.1到5.0秒之间'}), 400
        if output_format not in {'mp4', 'avi', 'webm'}:
            return jsonify({'code': 400, 'msg': '输出格式仅支持 mp4/avi/webm'}), 400

        images = request.files.getlist('images')
        if not images:
            return jsonify({'code': 400, 'msg': '未上传图片文件'}), 400
        if len(images) > MAX_BATCH_IMAGE_COUNT:
            return jsonify({'code': 400, 'msg': '图片数量超过限制（最多500张）'}), 400
        ...
        for img_file in images:
            ...
            if not allowed_file(img_file.filename):
                skipped_files.append(img_file.filename)
                continue
            try:
                img = read_image_file(img_file)
                if img is None:
                    skipped_files.append(img_file.filename)
                    continue
        ...
        if not image_list:
            return jsonify({'code': 400, 'msg': '无法读取任何有效图片'}), 400
        ...
        if not writer.isOpened():
            return jsonify({'code': 500, 'msg': '视频写入器初始化失败，请检查编码器支持'}), 500
        ...
        if output_format == 'mp4':
            print(f'[images_to_video] 开始MP4转码: {working_output_path} -> {output_path}')
            transcoded = transcode_mp4_for_browser(working_output_path, output_path)
            print(f'[images_to_video] 转码结果: {"成功(H.264)" if transcoded else "失败(使用原始编码)"}')
            if transcoded:
                try:
        ...
        if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
            return jsonify({'code': 500, 'msg': '视频生成失败，输出文件为空'}), 500
        ...
        if output_path and os.path.exists(output_path):
            try:
                ...
                pass
        return jsonify({'code': 500, 'msg': f'视频生成失败: {str(e)}'}), 500
...
def detect_video():
    ...
    try:
        if 'file' not in request.files:
            return jsonify({'code': -1, 'message': '???????'}), 400
        ...
        print(f"[DEBUG] pt_path: {pt_path}")
        if not pt_path:
            return jsonify({'code': -1, 'message': '?????????(pt_path)'}), 400
        ...
        if ext not in ALLOWED_VIDEO_EXTENSIONS:
            return jsonify({'code': -1, 'message': f'????????: {ext}???? {ALLOWED_VIDEO_EXTENSIONS}'}), 400

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        safe_filename = f"input_{timestamp}_{file.filename}"
        video_path = os.path.join(UPLOAD_FOLDER, safe_filename)
        file.save(video_path)
        print(f"[DEBUG] ?????: {video_path}, ??: {os.path.getsize(video_path)} bytes")
        ...
        print(f"[DEBUG] ????: {width}x{height}, {fps}fps, {total_frames}?, {duration:.2f}?")
        width, height = normalize_video_dimensions(width, height)

        print(f"[DEBUG] ????: {pt_path}")
        model = get_model(pt_path)
        ...
        out = cv2.VideoWriter(working_output_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height))
        if not out or not out.isOpened():
            cap.release()
            return jsonify({'code': -1, 'message': '??????????????????'}), 500

        print('[DEBUG] ????: mp4v')
        print(f"[DEBUG] ????: {result_path}")
        ...
        while True:
            ...
            if frame_count % 200 == 0:
                print(f"[DEBUG] ??? {frame_count}/{total_frames} ?...")

        cap.release()
        out.release()

        print(f"[DEBUG] ??????????? H.264: {working_output_path} -> {result_path}")
        ...
        if transcoded:
            try:
                ...
                pass
            print(f"[DEBUG] ?????????? H.264: {result_path}")
        else:
            os.replace(working_output_path, result_path)
            print(f"[DEBUG] ffmpeg??????? OpenCV ?? MP4 ??: {result_path}")

        result_file_size = os.path.getsize(result_path)
        print(f"[DEBUG] ??????: ??? {processed_count} ?, ??? {total_defect_count} ???")
        print(f"[DEBUG] ??????: {result_file_size} bytes, ??: {result_path}")

        processing_time = round((time.time() - start_time) * 1000, 2)

        max_frame_results = 2000
        if len(all_frame_defects) > max_frame_results:
            step = max(1, len(all_frame_defects) // max_frame_results)
        ...
        if has_any_defect and defect_type_counts:
            ...
            main_defect_type_cn = skin_defect_cn_mapping.get(main_defect_type, main_defect_type)
        ...
        print(f"??????: {processed_count}?, {total_defect_count}???, ??{processing_time}ms")
        return jsonify(response_data)
        ...
        import traceback
        traceback.print_exc()
        return jsonify({'code': -1, 'message': f'??????: {str(e)}'}), 500
    finally:
...
if __name__ == '__main__':
    ...
    print("支持检测类型: Benign-skin-cancer(良性皮肤肿瘤), Malignant-skin-cancer(恶性皮肤癌)")
    print("支持视频格式: mp4, avi, mov, mkv, wmv, flv, webm")
    print(f"上传目录: {UPLOAD_FOLDER}")
    print(f"结果目录: {RESULT_FOLDER}")
    print("接口地址: http://localhost:2026")
    print("=" * 60)
